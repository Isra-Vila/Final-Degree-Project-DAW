...
const BikeList: React.FC<BikeListProps> = memo(({
  ...
  onSelectBike,
  onAddBike,
  hideAddBikeButton = false, 
  customBorderColor = "border-[#F62364]" 
}) => (
          ...
          {bikes.map(bike => (
            <li
              ...
              className={`cursor-pointer rounded-md shadow-sm transition duration-150 ease-in-out ${
                selectedBikeId === bike.id ? 'bg-blue-600 text-white' : 'bg-white hover:bg-gray-50 text-gray-800' 
              }`}
              onClick={() => onSelectBike(bike)}
            >
              <div className="px-4 py-3">
                <h3 className="font-semibold text-md">{bike.brand}</h3>
                <p className="text-sm text-gray-300">{bike.model}</p> 
              </div>
            </li>
          ))}
        </ul>
