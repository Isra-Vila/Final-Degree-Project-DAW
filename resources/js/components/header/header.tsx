...
export default function Header({ userRole, isLoggedIn, setIsLoggedIn }: HeaderProps) {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 bg-[#F62364] shadow-md py-6 md:py-4`}>
      <motion.nav className="relative flex items-center justify-between text-white px-6">
        <button onClick={() => setOpen(!open)} className="md:hidden focus:outline-none z-10" aria-label="Menú móvil">
          {open ? (
            <svg
        ...
        <div
          className={`relative flex items-center gap-6 pr-6 transition-opacity duration-500 ${isHomePage ? (scrolled ? "opacity-100" : "opacity-0 pointer-events-none") : "opacity-100 pointer-events-auto"}`}
        >
      ...
      <div
        className={`md:hidden fixed inset-0 bg-[#F62364] z-40 transition-transform duration-300 ease-in-out transform ${open ? "translate-x-0" : "-translate-x-full"}`}
        style={{ top: "5rem" }}
      >
        {open && (
          <MobileNavLinks
